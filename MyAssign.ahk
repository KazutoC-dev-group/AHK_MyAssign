...
; 2026/02/19 UTF-8に変更
; 2026/02/19 時刻入力機能を追加（Ctrl+:）
; 2026/09/05 ホイールクリック（中クリック）でCtrl+W（閉じる）を送るようにした
;--------------------------------------------------------
; キーアサイン一覧 
; 無変換+v　		バージョン表示
; 無変換		IME を OFF
...
; Ctrl+:		時刻入力（hh:mm:ss形式）※エクセルを除く
; Ctrl+Shift+:		時刻入力（hhmmss形式）※エクセルを除く
...
vk1D & v::
{
	MsgBox ("MyAssign last update 2026/09/05")
	return
}
...
;  ホイールクリックでCtrl+W（閉じる）
;******************************************************************************
;中クリック本来の動作（オートスクロール、リンクを新しいタブで開く等）は無効になる
MButton::
{
	Send "^w"
	Return
